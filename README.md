...
## Getting Started
1. Ensure the Flutter SDK (3.24 or newer) is installed and configured.
...
## Platform Assets
- The iOS asset catalog currently contains empty placeholders so that no binary image files are required in source control. Add your production icons by dropping PNG files into `ios/Runner/Assets.xcassets/AppIcon.appiconset/` and `ios/Runner/Assets.xcassets/LaunchImage.imageset/`, then update each `Contents.json`.
- The web runner ships without bundled favicon assets. Reference your own icons from `web/index.html` and `web/manifest.json` when you are ready to brand the PWA build.
